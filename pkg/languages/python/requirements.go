...
var (
	...
	reqLineRe = regexp.MustCompile(`(?i)^([A-Z0-9]([A-Z0-9._-]*[A-Z0-9])?)(\[[^\]]*\])?\s*([><=!~^][^;#\n]*)?(.*)$`)

	// reqSpecifierRe splits an operator + version in a requirements line.
	reqSpecifierRe = regexp.MustCompile(`^([><=!~^]+)(.+)$`)

	// reqVersionRe matches the operator(s) and version in a requirement line for rewriting.
	reqVersionRe = regexp.MustCompile(`([><=!~^]+)\s*([0-9][^\s,;#]*)`)
)
...
func splitReqSpecifier(s string) (op, version string) {
	...
	m := reqSpecifierRe.FindStringSubmatch(s)
	if m == nil {
		return "", s
	}
...
func UpdateRequirement(path, packageName, newVersion string) error {
	...
	for i, raw := range lines {
		...
		if m[4] == "" {
			...
			replaced := false
			newSpec := reqVersionRe.ReplaceAllStringFunc(m[4], func(match string) string {
				if replaced {
					return match
				}
				replaced = true
				mm := reqVersionRe.FindStringSubmatch(match)
