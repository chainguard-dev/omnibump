...
func parsePoetryDeps(deps map[string]any) []VersionSpec {
	specs := make([]VersionSpec, 0, len(deps))
	for name, val := range deps {
		if name == LanguagePython {
			continue
		}
		var ver string
		switch v := val.(type) {
...
var (
	...
	pep508Re = regexp.MustCompile(`(?i)^([A-Z0-9]([A-Z0-9._-]*[A-Z0-9])?)\s*(?:\[[^\]]*\])?\s*([><=!~^][^;]*)?`)

	// specifierRe splits an operator + version string (e.g. ">=2.28.0").
	specifierRe = regexp.MustCompile(`^([><=!~^]+)(.+)$`)

	// pkgNameNormalizeRe replaces runs of hyphens, underscores, and dots per PEP 503.
	pkgNameNormalizeRe = regexp.MustCompile(`[-_.]+`)

	// pythonVersionRe validates version strings safe for manifest files.
	pythonVersionRe = regexp.MustCompile(`^[a-zA-Z0-9._+\-]+$`)

	// rebuildVersionRe matches operator + version for rewriting in PEP 508 strings.
	rebuildVersionRe = regexp.MustCompile(`([><=!~^]+)\s*([0-9][^\s,;"]*)`)

	// poetrySimpleRe matches: pkg = "specifier" in [tool.poetry.dependencies].
	poetrySimpleRe = regexp.MustCompile(`(?i)^(\s*)([A-Z0-9][A-Z0-9._-]*)(\s*=\s*")([><=!~^]*)([0-9][^"]*)(".*$)`)

	// poetryTableRe matches: pkg = {version = "specifier", ...} in [tool.poetry.dependencies].
	poetryTableRe = regexp.MustCompile(`(?i)^(\s*)([A-Z0-9][A-Z0-9._-]*)(\s*=\s*\{[^}]*version\s*=\s*")([><=!~^]*)([0-9][^"]*)(".*$)`)
)
...
func updatePoetryDep(data []byte, path, pkg, newVersion string) (bool, error) {
	...
	for i, line := range lines {
		for _, re := range []*regexp.Regexp{poetrySimpleRe, poetryTableRe} {
...
func rebuildPEP508(original string, spec *VersionSpec, newVersion string) string {
	...
	if strings.Contains(spec.Specifier, ",") {
		replaced := false
		return rebuildVersionRe.ReplaceAllStringFunc(original, func(match string) string {
			if replaced {
				return match
			}
			replaced = true
			m := rebuildVersionRe.FindStringSubmatch(match)
...
func normalizePkgName(name string) string {
	return pkgNameNormalizeRe.ReplaceAllString(strings.ToLower(name), "-")
}
