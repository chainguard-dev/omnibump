...
// pipfileLineRe matches a Pipfile dependency line: pkg = "specifier".
var pipfileLineRe = regexp.MustCompile(`(?i)^(\s*)([A-Z0-9][A-Z0-9._-]*)(\s*=\s*")([><=!~^]*)([0-9][^"]*)(".*$)`)
...
func ParsePipfile(data []byte) ([]VersionSpec, error) {
	...
	for _, deps := range map[string]map[string]any{
...
func UpdatePipfile(path, packageName, newVersion string) error {
	...
	for i, raw := range lines {
		m := pipfileLineRe.FindStringSubmatchIndex(raw)
		if m == nil {
			continue
		}
		name := raw[m[4]:m[5]]
